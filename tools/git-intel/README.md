...
The binary location is `tools/git-intel/target/release/git-intel` (relative to the pelley repository root). Other projects that install pelley access the binary via absolute path resolution.
...
This allows cross-project access when pelley is installed globally via symlinks.
...
The ONNX model lives at `tools/data/onnx-model/` (relative to pelley repository root) and contains:
...
2. The ONNX model exists at `tools/data/onnx-model/model.onnx` (checked in current repo root first, then pelley root)
...
Run `bin/git-pulse.sh` from your project root (or `~/.config/opencode/bin/git-pulse.sh` if pelley is installed globally):
