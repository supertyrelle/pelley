...
- Default to the most restrictive tool set that still allows the agent to function
- Read-only agents (reviewers, analyzers) must not have Write, Edit, or unrestricted Bash access
- When an agent needs broad shell access, document the justification in the agent's description
...
- Treat `~/.config/opencode/`, `~/.ssh/`, `~/.aws/`, and `~/.gnupg/` as sensitive -- never read from these directories unless the task explicitly requires it
