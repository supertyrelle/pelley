...
Teams are defined in `team.yaml` at the project root. This is the single source of truth for team composition.
...
| `tools` | member | Tool list declared in agent definition frontmatter, enforced advisorily by skill-guard.ts |
...
3. **Promote high-value entries**: If a learning has been confirmed across 3+ sprints, promote it to `rules/` or opencode.md
...
The orchestrator dispatches team members via the `spawn-agent` tool, which creates isolated SDK sessions.

### Dispatch via spawn-agent

Use `spawn-agent(agent="<name>", task="<description>", taskId="<bead-id>")`. If a matching agent definition exists in `agents/<name>.md`, spawn-agent reads its system prompt and creates an SDK session.
...
**For independent tasks:** Use `team-dispatch` tool for batch dispatch when tasks touch different ownership areas and have no shared files.
...
- Read learnings file, embed in the SDK session's system prompt
...
- Promote high-value entries to rules or opencode.md
...
- Injectable into agent prompts via SDK session dispatch
...
When no `team.yaml` exists, skills degrade gracefully to solo mode:

1. **No team.yaml required.** Skills that reference team.yaml degrade gracefully when it is absent. Use solo mode paths (e.g., `memory/agents/solo/`) instead of per-member paths.
2. **Spawn protocol uses spawn-agent.** All dispatch goes through `spawn-agent(agent=..., task=...)`. The prompt must be fully self-contained since there is no persistent agent context.
3. **Reflection schema is unchanged.** Spawned agents can still return structured reflections. The orchestrator parses and persists learnings to `memory/agents/solo/learnings.md`.
4. **Learning lifecycle degrades to seed/inject/reflect.** The prune step (normally run via `/retro` with team context) operates on the solo learnings file. Transfer between members is not applicable.
5. **Shared team memory is optional.** `memory/team/decisions.md` can still be used for decision logging even without a team manifest, but cross-agent notes have no target members to route to.
