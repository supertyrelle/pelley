...
How to organize knowledge, configuration, and documentation in projects so that context reaches the model at the right time with minimal retrieval cost.
...
Put the minimum viable context at the root; link to detail. opencode.md gives orientation. Rules give constraints. Docs give depth. A reader (human or model) should be able to stop at any level and still function.

### 3. Passive Context Over Active Retrieval

Prefer files that load automatically (opencode.md, rules/, hooks) over files that require a tool call to discover. Every retrieval step costs tokens and risks the model not looking. If something matters for every session, make it passive.
...
Root index files (opencode.md, AGENTS.md, INDEX.md) stay small — orientation only. Full content lives in the artifact itself. This keeps passive-context budgets tight and avoids the "mega-file" anti-pattern.
...
**When creating a new artifact:** Pick the right tier — passive (rules/), discoverable (docs/), or on-demand (deep in a subdirectory). Ask: "Does every session need this?" If yes, make it a rule. If most sessions, put it in opencode.md. If some sessions, put it in docs/ and link from an index.
