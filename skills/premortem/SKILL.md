...
Present the user with options for which scenarios to mitigate:
...
This skill works as-is in single-agent environments. The three failure lenses (Security, Reliability, User/Business) are generated sequentially via subagent dispatch with role rotation. The single agent adopts each lens in turn, producing equivalent output. No adaptation needed.
