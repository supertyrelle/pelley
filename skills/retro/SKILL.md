...
**If `git-pulse.sh` is available** (check `bin/git-pulse.sh`), run:

```bash
bin/git-pulse.sh --since="8 hours ago" 2>/dev/null
```
...
Be specific and actionable. "Communication was good" is not useful. "Returning spike reports with structured format via subagent dispatch made orchestrator processing 3x faster" is useful.
...
**Only run this phase if `team.yaml` exists.** Skip entirely for non-team sessions.
...
3. **Promote high-value entries** — **If Phase 4f promotion candidates were surfaced**, review that list and promote entries that survived 3+ retros and are stable 21+ days. **If Phase 4f data is NOT available**, fall back to manual review: promote entries confirmed across 3+ sprints to `rules/` or opencode.md where all agents benefit
...
4. **Surface promotion candidates** — List learnings that:
   ...
   - Are not already in `rules/` or `opencode.md`
