...
allowed-tools: Read, Write, Edit, Grep, Glob, Bash(bd:*), Bash(git:*)
---

# Team Meeting: Goal-Oriented Planning with Your Team

You are facilitating a **Team Meeting** -- a collaborative planning session where the persistent team (from `/assemble`) breaks down a goal into sprint-ready tasks with concrete member assignments. Each team member's perspective is generated by adopting their role, ownership, and accumulated learnings.
...
Load team manifest + learnings
  -> Generate each member's planning contribution (sequential)
    -> Cross-reference dependencies and conflicts
...
Read `.opencode/team.yaml`. If it does not exist, tell the user:
...
Read `.opencode/team.yaml` and extract all member names, roles, and ownership patterns.
...
## Phase 2: Generate Member Planning Reports
...
### 2b. Generate Each Member's Contribution

For each participating member, adopt their role and perspective (informed by their ownership areas and accumulated learnings) to produce a planning report covering:

1. **Their scope**: What specific work falls in their ownership area? Be concrete -- name files, modules, or systems they would touch.
2. **Task breakdown**: Break their scope into 1-4 discrete tasks. Each task should be completable in one agent session. For each task:
   - Title (action verb + specific thing)
   - What changes (files, functions, patterns)
   - Estimated complexity (small / medium / large)
   - Acceptance criteria (how to verify it is done)
3. **Dependencies**: What do they need from other team members before they can start? What do other members need from them?
4. **Risks**: What could go wrong in their area? What assumptions are they making?
5. **Questions**: Anything that needs clarification before committing to this plan?

Generate contributions sequentially, staying fully in character for each member.

---

## Phase 3: Facilitation

### 3a. Cross-Reference Dependencies
...
If dependencies, conflicts, or questions need resolution:

- **Develop each member's position** on the conflict, staying in their role's perspective
- **Anonymize sources** when presenting opposing views -- describe the concern, not who raised it
...
For questions that require user input, collect them and ask the user:
...
After user answers, incorporate them into the plan.
...
- Dependencies (from cross-reference in 3a)
...
1. **Use the real team.** Members come from `.opencode/team.yaml`, not abstract roles. Their ownership patterns and learnings shape their planning contributions.
...
5. **Dependencies are the hard part.** Most planning value comes from surfacing what members need from each other. The dependency cross-reference (Phase 3a) is the core of this skill.
