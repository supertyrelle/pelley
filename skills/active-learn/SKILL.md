...
allowed-tools: [Read, Write, Grep, Glob, WebSearch, WebFetch, "Bash(git:*)", "Bash(tools/git-intel/target/release/git-intel:*)", "Bash(bin/git-pulse.sh:*)", "Bash(mkdir:*)", "Bash(wc:*)"]
context: fork
---
...
Parse arguments (agent name, round count)
  -> Phase 1: Diagnose (inline profiling from learnings + git signals)
    -> Phase 2: Generate challenges (inline, 3-5 per round)
      -> Phase 3: Execute round (subagent dispatch, serial, evaluate inline)
...
1. Read `.opencode/team.yaml` and list available agent names
...
Determine operating mode by attempting to read `.opencode/team.yaml`:

**If `.opencode/team.yaml` exists AND the agent name is found in `members`:**
- Extract the agent's `role`, `model`, and `owns` patterns from the file
- Operating in **TEAM MODE** -- proceed normally

**If `.opencode/team.yaml` does not exist OR the agent name is not found in it:**
- Operating in **SOLO MODE**
- Print to the user: "SOLO MODE: .opencode/team.yaml not found or agent '<name>' not in team. Running as 'main-session'."
...
In team mode, read `.opencode/team.yaml` to extract the agent's `owns` patterns. In solo mode, no `owns` filtering applies -- analyze all project files.
...
## Phase 3: Execute Round (Dispatch + Inline Evaluation)

### 3a. Dispatch Each Challenge Serially

For each challenge, compose a self-contained prompt and dispatch to a subagent.
...
```
dispatch(agent="<agent-name>", task="<composed prompt>")
```

Serial dispatch only. Each challenge is a controlled experiment.
...
2. **Serial dispatch, serial evaluation.** Process one challenge at a time within a round. Each challenge is a controlled experiment.
