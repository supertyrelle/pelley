...
allowed-tools: Read, Grep, Glob, Bash(bd:*), Bash(git:*), Bash(mkdir:*), Write, Edit
---
...
- Read README, opencode.md, or other project docs if present
- Check package.json, Cargo.toml, pyproject.toml, or similar to identify stack
- Look for existing `team.yaml` (this may be a team update)
...
- README, opencode.md, package.json/Cargo.toml/pyproject.toml (or equivalent)
- Directory structure (`ls` top level, key subdirectories)
- Existing `team.yaml` if present (this may be a team update, not creation)
...
**Defaults**: model=sonnet, budget=$0.50
```
...
Write `team.yaml` following the format defined in the team-protocol rule:
