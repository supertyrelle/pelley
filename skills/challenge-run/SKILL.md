...
allowed-tools: Read, Write, Grep, Glob, "Bash(git:*)", "Bash(mkdir:*)", "Bash(wc:*)"
context: inline
---
...
2. If `$ARGUMENTS` is empty, list available agents from `.opencode/team.yaml` and ask the user which to target. Stop and wait.
...
1. Read `.opencode/team.yaml` and confirm the agent name exists in the `members` list
...
```
dispatch(agent="<agent-name>", task="<composed prompt from 1a>")
```

Use serial dispatch. Each challenge is independent but evaluation benefits from sequential processing.
