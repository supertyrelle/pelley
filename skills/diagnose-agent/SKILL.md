...
1. Read `team.yaml` and list available agent names
2. Ask the user: "Which agent should I diagnose? Available: [names]"
3. Stop and wait for response

If `$ARGUMENTS` is provided:
1. Read `team.yaml` and confirm the agent name exists in the `members` list
2. Check that `memory/agents/<name>/learnings.md` exists
3. If agent not found in team.yaml, error: "Agent '<name>' not found in team.yaml. Available agents: [list]."
...
Read `team.yaml` to extract the agent's `owns` patterns (file globs defining ownership).
