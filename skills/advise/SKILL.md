...
allowed-tools: [Read, Glob, Grep, "Bash(bd:*)", "Bash(git status:*)", "Bash(git log:*)", "Bash(git diff:*)", "Bash(bin/git-pulse.sh:*)", "Bash($HOME/.config/opencode/bin/git-pulse.sh:*)"]
context: inline
---
...
```bash
"$HOME/.config/opencode/bin/git-pulse.sh" 2>/dev/null || bin/git-pulse.sh 2>/dev/null
```
