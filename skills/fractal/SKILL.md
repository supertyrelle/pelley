...
argument-hint: "<goal>"
allowed-tools: Read, Grep, Glob, Bash(bd:*), Bash(git:*), Bash(mkdir:*), Write
---
...
Launch handlers as isolated subagents using the handler template below. Dispatch sequentially, processing each handler's results before launching the next.

```
dispatch(agent="explorer", task="<handler instructions>")
```
