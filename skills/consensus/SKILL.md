...
Frame the problem (same starting point for all agents)
  -> Select 3 optimization lenses (adaptive based on problem type)
    -> Solicit 3 perspectives (subagent dispatch, one per lens)
...
### 2a. Dispatch Three Agents Sequentially

Dispatch three subagents sequentially, each in an isolated context. Give each agent the same problem statement, but different optimization instructions based on the lenses selected in Phase 1d.
...
```
dispatch(agent="advocate", task="[Lens Name] Advocate: <full prompt>")
```

### 2b. Wait for All Three Reports

Process each agent's results as they complete. Do not proceed until all three have returned their proposals.
...
Dispatch three subagents sequentially (one per original lens), each in an isolated context. Each agent receives:
...
Process all three debate agents' results before proceeding.
...
If there are tensions where no clear winner exists based on constraints, ask the user for input:
...
- **Fresh agents prevent contamination**: Using separate isolated subagent contexts ensures one perspective doesn't influence another.
...
- **This is expensive**: Three sequential agent dispatches reading the codebase independently. Use this for meaningful decisions, not trivial choices. Debate round doubles the agent count to six.
