#!/usr/bin/env bash
# session-start.sh: OpenCode session start orientation card
...
# --- Team info ---
if [ -f .opencode/team.yaml ]; then
  TN=$(grep '^team:' .opencode/team.yaml 2>/dev/null | head -1 | sed 's/^team: *//')
  MC=$(grep '^ *- name:' .opencode/team.yaml 2>/dev/null | wc -l | tr -d ' ')
