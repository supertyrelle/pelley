...
A comprehensive guide to the persistent learning team system for pelley — a framework for creating project teams where agents accumulate knowledge and improve across sessions.
...
A YAML file at `team.yaml` defining:
- Team name and description
- Default settings (model, budget)
...
- **Tools**: Allowed tools declared in agent definition frontmatter, enforced advisorily by skill-guard.ts
...
When you dispatch a team member (via `/sprint` or manually via spawn-agent), the orchestrator:
...
- `rules/` (if it's a rule all agents should follow)
- `opencode.md` (if it's fundamental project context)
...
**Fix:**
- Edit `team.yaml` to fix the member definition
...
If a learning has been confirmed across 3+ sprints, promote it to `rules/` or `opencode.md` so all agents (not just one role) benefit.
