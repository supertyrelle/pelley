...
Global artifacts (`rules/`, `skills/`, `agents/`) are symlinked to `~/.config/opencode/` by `install.sh` and load in every project. Project-local artifacts (`.opencode/` directory) only load when working in this repo. This separation means a skill like /gather works in any project, while a project-local agent only appears when editing this repo. The alternative — everything global — would pollute other projects with pelley-specific tooling.
...
**Date:** 2025-02
**Context:** The primary session was doing everything: reading code, writing code, managing backlog, dispatching agents.
