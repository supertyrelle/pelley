...
<!-- Derived from PELLEY.md -- edit PELLEY.md for shared content, this file for Claude Code specifics -->

Portable workflow definitions (agents, skills, hooks, settings) maintained in a single git repo and installed to `~/.claude/` via symlinks.
...
xargs rm -f < ~/.claude/.pelley.manifest
...
- **Hooks**: Lifecycle hooks are defined in `settings.json` (SessionStart, PreCompact, PreToolUse, PostToolUse, UserPromptSubmit, SessionEnd).
- **MCP servers**: Configured via `claude mcp add --scope user`. Reference definitions live in `mcp-servers.json`. Runtime config lives in `~/.claude.json` (not symlinked).
...
- **Skill frontmatter**: `name`, `description`, `allowed-tools`, `context`, `disable-model-invocation`

### Settings Files

- `settings.json` at repo root is the **global** settings file, symlinked to `~/.claude/settings.json`. Contains hooks and env vars that apply to ALL projects.
...
│   ├── agents/                 # Project-local agents (authoring, research, maintenance)
...
- `settings.json` at repo root is the **global** settings file symlinked to `~/.claude/settings.json`. It contains hooks (SessionStart, PreCompact, PreToolUse, PostToolUse, UserPromptSubmit, SessionEnd) and env vars that apply to ALL projects.
- `.claude/settings.json` is the **project-local** settings file for working on this repo itself.
