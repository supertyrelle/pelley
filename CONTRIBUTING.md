# Contributing to pelley

Thanks for your interest in contributing. This guide covers the basics.

## Project Structure

pelley is a **content-only repo** -- Markdown definitions, JSON config, and TypeScript plugins. No application source code. The main artifact types:

- **Skills** (`skills/<name>/SKILL.md`) -- Workflow definitions invoked as tools
- **Agents** (`agents/<name>.md`) -- Agent definitions with YAML frontmatter
- **Rules** (`rules/<name>.md`) -- Global behavioral constraints
- **Plugins** (`.opencode/plugins/*.ts`) -- Lifecycle hooks and safety guards
- **Tools** (`.opencode/tools/*.ts`) -- Custom tool definitions
...
1. Create `.opencode/tools/<name>.ts`
2. Export a `tool()` definition with description, args schema, and execute function
3. Available after restart -- no install.sh needed

## Adding a Plugin

1. Create `.opencode/plugins/<name>.ts`
2. Export a `Plugin` function that hooks into lifecycle events
3. Available after restart -- no install.sh needed
...
Since this is a content-only repo, "testing" means running your skill or agent in a live OpenCode session:

1. Run `./install.sh` to symlink your changes
2. Open an OpenCode session in any project
3. Invoke your skill and verify it works
